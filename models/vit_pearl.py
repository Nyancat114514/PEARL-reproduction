...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., n_tasks=10):
        super().__init__()
    ...
    # Inside Attention_LoRA class
    def reinitialize_lora_for_task(self, task_idx, target_is_value=False):
        """
    ...
    def add_lora_for_task(self, task_idx, rank=64, rank_v_new=None):
        """
        Adds LoRA layers for a specific task with dynamically determined ranks.
        """
        device = self.qkv.weight.device

        # Ensure ModuleLists are long enough
        while len(self.lora_A_k) <= task_idx:
    ...
    def get_current_task_lora_weights(self, task_idx, target_is_value=False):
        ...
        if not target_is_value:
            if task_idx < len(self.lora_A_k) and self.lora_A_k[task_idx] is not None:
                return self.lora_B_k[task_idx].weight @ self.lora_A_k[task_idx].weight
        # else: (for value)
        return None
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention_LoRA(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, n_tasks=n_tasks)
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), task)))
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,n_tasks=n_tasks)
